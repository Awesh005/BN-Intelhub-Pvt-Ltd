...
class PointerHandler {
  ...
  constructor(element: HTMLCanvasElement, scale: number) {
    ...
    const map = (x: number, y: number) => [
      x * this.getScale(),
      element.height - y * this.getScale(),
    ];
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resize = () => {
        ...
        canvasRef.current.width = Math.max(1, rect.width * currentDpr);
        canvasRef.current.height = Math.max(1, rect.height * currentDpr);
        rendererRef.current?.updateScale(currentDpr);
        pointersRef.current?.updateScale(currentDpr);
      };
...
export function ShaderBackground({ className }: ShaderBackgroundProps) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn('absolute inset-0 h-full w-full touch-none object-cover', className)}
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
          ...
          <div className="mx-auto max-w-5xl space-y-6 px-4 text-center">
            <div className="space-y-2">
              <h1 className="shader-animate-fade-in-up shader-delay-200 bg-gradient-to-r from-orange-300 via-yellow-400 to-amber-300 bg-clip-text text-5xl font-bold text-transparent md:text-7xl lg:text-8xl">
                {headline.line1}
              </h1>
              <h1 className="shader-animate-fade-in-up shader-delay-400 bg-gradient-to-r from-yellow-300 via-orange-400 to-red-400 bg-clip-text text-5xl font-bold text-transparent md:text-7xl lg:text-8xl">
                {headline.line2}
              </h1>
            </div>

            <div className="shader-animate-fade-in-up shader-delay-600 mx-auto max-w-3xl">
            ...
            {buttons && (
              <div className="shader-animate-fade-in-up shader-delay-800 mt-10 flex flex-col justify-center gap-4 sm:flex-row">
                {buttons.primary && (
                  <button
